...
const { royaltymodel } = require('../Baseconfg/Royalty');
const fs = require('fs');
...
test('Save query to SQL file', async () => {
  const RoyaltyA = new royaltymodel({ deal_sid: 4135 });
  const queryText = RoyaltyA.ScopeA();
  ...
  expect(result?.length ?? 0).toBeGreaterThan(
    0,
    `❌ Test failed: deal_sid ${RoyaltyA.config.deal_sid} is incorrect (no data returned).`
  );
...
test('Run SQL and compare Known vs Test', async () => {
  ...
  console.log(`✅ Rows returned for deal_sid ${RoyaltyA.config.deal_sid}:`, result.length);
  ...
  const reportPath = path.join(
    ...
    'Test Results',
    'Reports',
    'qc_compare_ScopeA_mismatches.json'
  );
  fs.mkdirSync(path.dirname(queryFilePath), { recursive: true });
